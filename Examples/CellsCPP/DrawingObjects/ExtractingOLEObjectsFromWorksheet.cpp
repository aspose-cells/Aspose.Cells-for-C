...
void ExtractingOLEObjectsFromWorksheet()
{
	...
	Aspose::Cells::System::Int32 oleCount = worksheet->GetIOleObjects()->GetCount();
	...
	for (int i = 0; i < oleCount; i++)
	{
		...
		intrusive_ptr<Aspose::Cells::System::String> strProgId = oleObj->GetProgID();

		// Find the correct file extension.
		intrusive_ptr<Aspose::Cells::System::String> fileExt = NULL;

		if (strProgId->Equals(new String("Document")) == true)
		{
			fileExt = new Aspose::Cells::System::String(".docx");
		}
		else if (strProgId->Equals(new String("Presentation")) == true)
		{
			fileExt = new Aspose::Cells::System::String(".pptx");
		}
		else if (strProgId->Equals(new String("Acrobat Document")) == true)
		{
			fileExt = new Aspose::Cells::System::String(".pdf");
		}

		// Find the correct file name with file extension.
		intrusive_ptr<Aspose::Cells::System::String> fileName = outDir->StringAppend(new String("outputExtractOleObject"))->StringAppend(fileExt);

		// Write the Ole object data with correct file name.
		intrusive_ptr<Aspose::Cells::System::IO::FileStream> fout = new Aspose::Cells::System::IO::FileStream(fileName, FileMode::FileMode_OpenOrCreate, FileAccess::FileAccess_Write);
